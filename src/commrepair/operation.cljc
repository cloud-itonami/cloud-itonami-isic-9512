(ns commrepair.operation
  "OperationActor -- one communication-equipment-repair operation = one
  ...
  One graph run = one communication-equipment-repair operation (intake
  ...
  `:repair/complete`/`:device/return` ALWAYS reach this node when the
  governor is clean -- see `commrepair.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [commrepair.repairopsllm :as repairopsllm]
...
(defn build
  ...
  `commrepair.store/Store`).
  opts:
    :advisor      -- a `commrepair.repairopsllm/Advisor` (default: mock-advisor)
