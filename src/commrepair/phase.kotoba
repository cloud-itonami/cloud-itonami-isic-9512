(ns commrepair.phase
  "Phase 0->3 staged rollout -- the communication-equipment-repair
  analog of `cloud-itonami-isic-9521`'s `repairshop.phase`.
    ...
    Phase 2  assisted-assess  -- adds jurisdiction assessment + safety
                                 screening + data-consent screening
  ...
  technician/shop owner's call. `commrepair.governor`'s `:actuation/
  complete-repair`/`:actuation/return-device` high-stakes gate
  enforces the same invariant independently -- two layers, not one,
  agree on this. `:safety/screen`/`:dataconsent/screen` are likewise
  ...
  op has. Like `repairshop.phase`'s (and every prior sibling's) phase
  3 `:auto` set, this domain has only ONE member (`:ticket/intake`) --
...
(def write-ops #{:ticket/intake :jurisdiction/assess :safety/screen
                 :dataconsent/screen :repair/complete :device/return})
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                                              :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:ticket/intake :jurisdiction/assess :safety/screen :dataconsent/screen}        :auto #{}}
