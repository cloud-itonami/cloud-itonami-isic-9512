(ns commrepair.registry
  "Pure-function repair-completion + device-return record construction
  -- an append-only communication-equipment-repair-shop book-of-record
  draft.
  ...
  `commrepair.facts` uses.

  `parts-cost-matches-claim?` is an HONEST, LITERAL reuse of
  `repairshop.registry/parts-cost-matches-claim?`'s own EXACT-MATCH
  ...
  quantity x parts-unit-price) reapplied to communication-equipment
  repair tickets rather than consumer-electronics repair tickets --
  not claimed as new.
  ...
  returning a device itself (that is `commrepair.operation`'s
...
(defn parts-cost-matches-claim?
  ...
  is an honest, literal reuse of `repairshop.registry`'s own check,
  not a new concept."
...
(defn register-repair-completion
  ...
  the RECORD a shop would keep. `commrepair.governor` independently
...
(defn register-device-return
  ...
  system; it builds the RECORD a shop would keep. `commrepair.
  governor` independently re-verifies the ticket's own post-repair
  safety-test status and customer-data-consent status, and blocks a
