(ns commrepair.sim
  ...
  customer-data-consent screening -> repair-completion proposal
  (always escalates) -> human approval -> commit, then through device-
  return proposal (always escalates) -> human approval -> commit, then
  shows five HARD holds (a jurisdiction with no spec-basis, a claimed
  parts cost that doesn't match parts-quantity times unit-price, a
  failed post-repair safety test, an unconfirmed customer-data-access
  consent [screened directly via `:dataconsent/screen` -- never via an
  actuation op against an unscreened ticket -- see this actor's own
  ...
  `recreation`'s, `sportsclub`'s, `partyops`'s and `memberorg`'s
  ADR-0001s already recorded], and a double completion/return of an
  already-processed ticket) that never reach a human at all, and
  prints the audit ledger + the draft repair-completion and device-
  return records."
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (println "== dataconsent/screen ticket-1 (clean; escalates -- human approves) ==")
    (println (exec! actor "t4" {:op :dataconsent/screen :subject "ticket-1"} operator))
    ...
    (let [r (exec! actor "t6" {:op :device/return :subject "ticket-1"} operator)]
      (println r)
      (println "-- human repair technician approves --")
    ...
    (println "== dataconsent/screen ticket-5 (unconfirmed customer-data consent -> HARD hold, never reaches a human) ==")
    (println (exec! actor "t11" {:op :dataconsent/screen :subject "ticket-5"} operator))
    ...
    (println (exec! actor "t13" {:op :device/return :subject "ticket-1"} operator))
