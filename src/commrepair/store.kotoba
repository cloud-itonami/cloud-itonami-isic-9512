(ns commrepair.store
  "SSoT for the communication-equipment-repair actor, behind a `Store`
  ...
  (test/commrepair/store_contract_test.clj), which is the whole point:
  the actor, the Repair Shop Governor and the audit ledger never know
  which SSoT they run on.

  Like `repairshop.store`'s own dual repair-completion/device-return
  history (itself following `marketadmin.store`'s/`registrar.store`'s/
  `wagering.store`'s dual-history precedent), this actor has TWO
  ...
  customer-data consent, which repair was completed, which device was
  returned, on what jurisdictional basis, approved by whom' is always
  a query over an immutable log -- the audit trail a customer trusting
  a repair shop needs, and the evidence an operator needs if a repair
  or a return is later disputed."
...
(defprotocol Store
  ...
  (dataconsent-screening-of [s ticket-id] "committed customer-data-consent screening verdict for a ticket, or nil")
  (assessment-of [s ticket-id] "committed jurisdiction assessment, or nil")
  (ledger [s])
  (completion-history [s] "the append-only repair-completion history (commrepair.registry drafts)")
  (return-history [s] "the append-only device-return history (commrepair.registry drafts)")
...
(defn demo-data
  ...
  []
  {:tickets
   {"ticket-1" {:id "ticket-1" :customer "Sakura Tanaka" :device "Smartphone (battery replacement)" :device-type :smartphone
                 :parts-quantity 2 :parts-unit-price 15 :claimed-parts-cost 30.0
                 :safety-test-passed? true :customer-data-consent-confirmed? true
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-2" {:id "ticket-2" :customer "Atlantis Doe" :device "Two-way radio" :device-type :radio
                 :parts-quantity 1 :parts-unit-price 40 :claimed-parts-cost 40.0
                 :safety-test-passed? true :customer-data-consent-confirmed? true
                 :repair-completed? false :device-returned? false
                 :jurisdiction "ATL" :status :intake}
    "ticket-3" {:id "ticket-3" :customer "鈴木一郎" :device "Smartphone (screen replacement)" :device-type :smartphone
                 :parts-quantity 2 :parts-unit-price 15 :claimed-parts-cost 50.0
                 :safety-test-passed? true :customer-data-consent-confirmed? true
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-4" {:id "ticket-4" :customer "田中花子" :device "Smartphone (charging-port repair)" :device-type :smartphone
                 :parts-quantity 1 :parts-unit-price 20 :claimed-parts-cost 20.0
                 :safety-test-passed? false :customer-data-consent-confirmed? true
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}
    "ticket-5" {:id "ticket-5" :customer "佐藤次郎" :device "Smartphone (data-recovery diagnostics)" :device-type :smartphone
                 :parts-quantity 1 :parts-unit-price 20 :claimed-parts-cost 20.0
                 :safety-test-passed? true :customer-data-consent-confirmed? false
                 :repair-completed? false :device-returned? false
                 :jurisdiction "JPN" :status :intake}}})
...
(defrecord MemStore [a]
  ...
  (dataconsent-screening-of [_ id] (get-in @a [:dataconsent-screenings id]))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :dataconsent-screening/set
      (swap! a assoc-in [:dataconsent-screenings (first path)] payload)
...
(defn seed-db
  ...
  (->MemStore (atom (assoc (demo-data)
                           :assessments {} :safety-screenings {} :dataconsent-screenings {}
...
(def ^:private schema
  ...
  Map/compound values (assessment/safety-screening/dataconsent-
  ...
  {:ticket/id                     {:db/unique :db.unique/identity}
   ...
   :dataconsent-screening/ticket-id {:db/unique :db.unique/identity}
...
(defn- ticket->tx [{:keys [id customer device device-type parts-quantity parts-unit-price claimed-parts-cost
                          safety-test-passed? customer-data-consent-confirmed? repair-completed? device-returned?
  ...
  (cond-> {:ticket/id id}
    ...
    (some? customer-data-consent-confirmed?)                   (assoc :ticket/customer-data-consent-confirmed? customer-data-consent-confirmed?)
...
(def ^:private ticket-pull
  [:ticket/id :ticket/customer :ticket/device :ticket/device-type :ticket/parts-quantity
   ...
   :ticket/customer-data-consent-confirmed? :ticket/repair-completed? :ticket/device-returned?
...
(defn- pull->ticket [m]
  (when (:ticket/id m)
    {:id (:ticket/id m) :customer (:ticket/customer m) :device (:ticket/device m)
     ...
     :customer-data-consent-confirmed? (boolean (:ticket/customer-data-consent-confirmed? m))
...
(defrecord DatomicStore [conn]
  ...
  (safety-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
              ...
              (d/db conn) id)))
  (dataconsent-screening-of [_ id]
    (dec* (d/q '[:find ?p . :in $ ?tid
                :where [?k :dataconsent-screening/ticket-id ?tid] [?k :dataconsent-screening/payload ?p]]
              (d/db conn) id)))
  ...
  (commit-record! [s {:keys [effect path value payload]}]
    (case effect
      ...
      :dataconsent-screening/set
      (d/transact! conn [{:dataconsent-screening/ticket-id (first path) :dataconsent-screening/payload (enc payload)}])
