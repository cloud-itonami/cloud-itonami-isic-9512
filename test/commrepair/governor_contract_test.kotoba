(ns commrepair.governor-contract-test
  "The governor contract as executable tests -- the communication-
  equipment-repair analog of `cloud-itonami-isic-9521`'s `repairshop.
...
(deftest customer-data-consent-unconfirmed-is-held-and-unoverridable
  (testing "an unconfirmed customer-data-access consent on a ticket -> HOLD, and never reaches request-approval -- exercised via :dataconsent/screen DIRECTLY, not via an actuation op against an unscreened ticket (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / eldercare's, museum's, conservation's, salon's, entertainment's, casework's, hospital's, facility's, school's, association's, leasing's, behavioral's, secondary's, card's, water's, telecom's, aerospace's, recovery's, consulting's, union's, congregation's, fab's, energy's, care's, navigator's, learning's, banking's, advertising's, polling's, research's, design's, nursing's, sports's, alliedhealth's, laundry's, holdco's, photo's, personalservice's, edsupport's, headoffice's, residential's, cultural's, reserve's, proserv's, sportsevent's, recreation's, sportsclub's, partyops's and memberorg's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :dataconsent/screen :subject "ticket-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:customer-data-consent-unconfirmed} (-> (store/ledger db) first :basis)))
      (is (nil? (store/dataconsent-screening-of db "ticket-5")) "no clearance written"))))
...
(deftest device-return-always-escalates-then-human-decides
  (testing "a clean, fully-assessed, passed-safety-test, confirmed-consent ticket still ALWAYS interrupts for human approval -- actuation/return-device is never auto"
    (let [[db actor] (fresh)
